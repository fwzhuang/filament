...
int main(int argc, char** argv) {
    ...
    auto animate = [&app](Engine* engine, View* view, double now) {
        if (app.animator->getAnimationCount() > 0 && app.currentAnimation > 0) {
            app.animator->applyAnimation(app.currentAnimation - 1, now);
        }
    ...
    auto gui = [&app](filament::Engine* engine, filament::View* view) {
        ...
        auto animationsTreeItem = [&]() {
            size_t count = app.animator->getAnimationCount();
            ImGui::RadioButton("Disable", &app.currentAnimation, 0);
            for (size_t i = 0; i < count; ++i) {
                std::string label = app.animator->getAnimationName(i);
                if (label.empty()) {
                    label = "Unnamed " + std::to_string(i);
                }
                ImGui::RadioButton(label.c_str(), &app.currentAnimation, i + 1);
            }
        };
